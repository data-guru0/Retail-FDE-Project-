  ...
  return (
    <div style={{ maxWidth: 480, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <input className="input" value={form.zip} onChange={set("zip")} placeholder="ZIP" />
      </div>
      <label>Card (test — no real charge, Luhn-checked)</label>
      <input className="input" value={form.card_number} onChange={set("card_number")} />
      <div style={{ display: "flex", gap: 8 }}>
        <input className="input" value={form.card_exp} onChange={set("card_exp")} placeholder="MM/YY" />
        <input className="input" value={form.card_cvc} onChange={set("card_cvc")} placeholder="CVC" />
      </div>
